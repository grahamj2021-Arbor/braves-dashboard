...
function renderStats(d){
  ...
  document.getElementById('content').innerHTML=`
    ...
    ${pitchers.length?`<div class="card">
      ...
      <div class="plist">${pitchers.map(p=>`<div class="prow"><div class="ptop"><div class="prn">${pcEsc(p.name)}</div><span class="prp ${pcEsc(p.pos).toLowerCase()}">${pcEsc(p.pos)}</span></div><div class="prstats"><div class="pri"><div class="pril">ERA</div>${badge(p.era,MLB_PIT.era,true,v=>v.toFixed(2))}</div><div class="pri"><div class="pril">WHIP</div>${badge(p.whip,MLB_PIT.whip,true,v=>v.toFixed(2))}</div><div class="pri"><div class="pril">K/9</div>${badge(p.k9,MLB_PIT.k9,false,v=>v.toFixed(1))}</div><div class="pri"><div class="pril">BB/9</div>${badge(p.bb9,MLB_PIT.bb9,true,v=>v.toFixed(1))}</div><div class="pri"><div class="pril">FIP</div>${badge(p.fip,MLB_PIT.fip,true,v=>v.toFixed(2))}</div><div class="pri"><div class="pril">${p.pos==='RP'?'SV':'W'}</div><span class="badge bg">${p.pos==='RP'?p.sv:p.w}</span></div></div></div>`).join('')}</div>
    </div>`:''}
